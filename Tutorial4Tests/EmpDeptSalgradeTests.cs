...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldJoinEmployeesWithDepartments()
    {
        ...
        var result = emps.Join(depts, e =>e.DeptNo, dept => dept.DeptNo, (e,d) => new
        {
           e.EName, d.DName  
        }).ToList(); 
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.GroupBy(e => e.DeptNo).Select(s => new { DeptNo = s.Key, Count = s.Count()}).ToList();
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var grades = Database.GetSalgrades();
         ...
         var result = emps.SelectMany(e => grades
             .Where(s => e.Sal >= s.Losal && e.Sal <= s.Losal), (e,s) => new {e.EName,s.Grade}).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        var emps = Database.GetEmps();
         ...
         var result = emps.GroupBy(e => e.DeptNo).Select(s => new { DeptNo =s.Key, AvgSal = s.Average(e => e.Sal)}).ToList(); 
